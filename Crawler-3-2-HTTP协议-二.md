...
![](Crawler-3-2-HTTP协议-二/feihua.jpg)
...
像这样：
![baowen1](Crawler-3-2-HTTP协议-二/baowen1.png)

详细研究下请求首部字段，接收到客户端请求的服务器，会用下面的处理结果以响应的形式返回
![return](Crawler-3-2-HTTP协议-二/return.png)
解释下服务器返回的结果：
- HTTP/1.1 表示HTTP的版本
...
响应报文基本上由上述四个模块组成
![return_constructure](Crawler-3-2-HTTP协议-二/return_constructure.png)

## HTTP是不保存状态的协议
HTTP的另一个性质是不保存状态stateless协议，HTTP协议自身不对请求和响应之间的通信状态进行保存，也就是HTTP这个级别的协议对于发送过请求和相应都没有持久化处理（可以理解为：处理完就忘了）
![stateless](Crawler-3-2-HTTP协议-二/stateless.png)
...
![uri](Crawler-3-2-HTTP协议-二/uri.png)
使用uri的方法有两种，一种是直接写完整的uri：
![hole_uri](Crawler-3-2-HTTP协议-二/hole_uri.png)
另一种是写成Host加相对地址的方式，host可以使域名或者ip地址
![host_uri](Crawler-3-2-HTTP协议-二/host_uri.png)
...
![trace](Crawler-3-2-HTTP协议-二/trace.png)
8. CONNECT 用隧道协议连接代理，实现用隧道协议进行TCP，主要使用SSL合同TLS协议吧通信内容加密后经网络隧道传输
![connect1](Crawler-3-2-HTTP协议-二/connect1.png)
![connect2](Crawler-3-2-HTTP协议-二/connect2.png)


## 使用方法下达命令
方法是一种命令，给uri对应的资源的，在报文中传送这条命令，方法可以指定请求的资源按期望产生某种行为，方法中有GET、POST和HEAD等
![command](Crawler-3-2-HTTP协议-二/command.png)

下面我们列举一些方法，注意区分大小写：
![command2](Crawler-3-2-HTTP协议-二/command2.png)
## 使用cookie的状态管理
...
![forgetthem](Crawler-3-2-HTTP协议-二/forgetthem.png)
...
![no_cookie](Crawler-3-2-HTTP协议-二/no_cookie.png)

![cookie_2](Crawler-3-2-HTTP协议-二/cookie_2.png)
没有Cookie的报文

![no_cookie_post](Crawler-3-2-HTTP协议-二/no_cookie_post.png)

![cookie_post](Crawler-3-2-HTTP协议-二/cookie_post.png)
