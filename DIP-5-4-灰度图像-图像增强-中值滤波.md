...
## 数学原理
中值滤波的数学公式就是 $g(x,y)=Median\{f(x-m/2,y-n/2).......f(x+m/2,y+n/2)\}$，翻译成自然语言就是在当前模板覆盖范围内需找中位数作为结果。
此计算中涉及到排序，所以，如果使用比较的方法排序，最快速度的复杂度是 $O(m*n*log(m*n)*W*H)$ ，如果使用非比较型排序，算法最大时间复杂度是 $255*W*H$ 也就是 $O(W*H)$ 但要使用更多的存储空间，最直观的方法是使用计数排序，建立一个255大小的空间，或者理解为一个直方图，来查找中值。
