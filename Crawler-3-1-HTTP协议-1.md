...
这种规则称之为协议：
![protocol](Crawler-3-1-HTTP协议-1/protocol.png)
...
### 传输流
![传输流](Crawler-3-1-HTTP协议-1/传输流.png)
...
![传输过程](Crawler-3-1-HTTP协议-1/传输过程.png)
...
![mac_arp](Crawler-3-1-HTTP协议-1/mac_arp.png)
### 可靠的TCP协议
为了确保到达目标TCP进行三次握手，确认无误：
![三次握手](Crawler-3-1-HTTP协议-1/三次握手.png)
当然TCP还有别的策略保证通讯准确无误
## DNS域名解析
...
![dns](Crawler-3-1-HTTP协议-1/dns.png)
## HTTP与各个协议
HTTP使用时，下面的简图可以大致反应各个协议发挥的作用：


![httpiptcp](httpiptcp.png)
...
可标识的对象，也称为标识符
举几个例子：
![uri](Crawler-3-1-HTTP协议-1/uri.png)
URI表示的互联网的某一个资源
...
![uri_form](Crawler-3-1-HTTP协议-1/uri_form.png)
- http:
