...
<!--more-->
## 开篇废话
废话不多说，但是本环节保留，最近这几篇应该是CUDA最核心的部分，并不是编程模型，而是执行模型，通过执行模型我们去了解GPU硬件的具体运行方式，这样才能保证我们写出更快更好的程序。
由于访问量太少，转载请保留本条广告，各位老铁欢迎访问Tony的网站：[http://www.tony4ai.com](http://www.tony4ai.com)
## 资源分配
...
![3_13](CUDA-F-3-2-理解线程束执行的本质-P2/3_13.png)

关于共享内存的分配：

![3_14](CUDA-F-3-2-理解线程束执行的本质-P2/3_14.png)
...
![3_2](CUDA-F-3-2-理解线程束执行的本质-P2/3_2.png)
...
![3_15](CUDA-F-3-2-理解线程束执行的本质-P2/3_15.png)
...
![3_16](CUDA-F-3-2-理解线程束执行的本质-P2/3_16.png)
下面表格给出了Fermi 和Kepler执行某个简单计算时需要的并行操作数：
![3_3](CUDA-F-3-2-理解线程束执行的本质-P2/3_3.png)
...
得出下表的数据
![3_4](CUDA-F-3-2-理解线程束执行的本质-P2/3_4.png)
...
```
结果
![devInf](CUDA-F-3-2-理解线程束执行的本质-P2/devinf.png)

最大64个线程束每个SM。
CUDA工具包中提供一个叫做UCDA占用率计算器的电子表格，填上相关数据可以帮你自动计算网格参数：
![excel](CUDA-F-3-2-理解线程束执行的本质-P2/excel.png)
...
![3_18](CUDA-F-3-2-理解线程束执行的本质-P2/3_18.png)
## 总结
