        ...
        for link in get_links(html):
            if re.search(link_regex,link):
                ...
                if link not in seen:
                    ...
                    crawl_queue.append(link)
```
上面使用的是[三只虫](http://tony4ai.com/Crawler-3-0-第一个爬虫/)里面的代码，其中这两部用到了正则表达式，一个用于提取，一个用于确定，可以看出，正则表达式其实不是很通用，每个网站甚至每个网页都要对应自己的表达式，所以这个基本算是半自动模式，过段时间如果网页代码发生改变，那么正则表达式可能会失效。
### Beautiful Soup
...
之前我们讲到的[三只虫](http://tony4ai.com/Crawler-3-0-第一个爬虫/)，只讲到如何访问网站所有的网页，这相当于我们转悠了一圈啥也没干，如果要处理点什么问题，比如下载个图片什么的，就要加入响应的功能，为了写一个通用的遍历网站的爬虫，处理问题部分用一个单独的函数，而这个函数作为参数传给遍历网站的模块，这种方法在C++中叫函数指针，这里我们可以称之为回调函数。
再次说明下，我们有一个框架，这个框架可以访问网站中的所有网页，但是框架为了通用性，没有设置每一页具体完成什么动作，而这个动作留作接口供大家发挥，这个接口就是回调函数（C++叫函数指针）

下面我们改造下[三只虫](http://tony4ai.com/Crawler-3-0-第一个爬虫/)中的代码，加入回调函数：
```python
